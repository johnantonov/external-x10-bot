...
export const texts = {
  ...
  addedSkuOld: "✅ SKU и расходы по ним внесены",
  ...
  warningChangeKey: `❗️ Если вы подключите ключ от другого личного кабинета, то перестанете получать отчеты по текущим артикулам.\n\nДля продолжения отправьте в чат обновленный ключ или ключ от другого кабинета.\n\nПереходите в настройки API (https://seller.wildberries.ru/supplier-settings/access-to-api) - Создать новый токен - Имя токена любое, например "10X" - выбрать абсолютно все галочки и установить флажок "только на чтение".\n\nГотовый ключ пришлите в этот чат:`,
